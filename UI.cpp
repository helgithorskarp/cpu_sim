...
static ImGuiTableFlags processes_table_flags = ImGuiTableFlags_BordersV |
                                               ImGuiTableFlags_Resizable |
                                               ImGuiTableFlags_RowBg;
static ImGuiTableFlags results_table_flags =
...
void SetDockspace() {
  ...
  ImGuiID dockspace_id =
      ImGui::GetID("MyDockSpace"); // Give it a unique string name
  ...
  if (ImGui::DockBuilderGetNode(dockspace_id) == nullptr) {
    ...
    ImGuiID dock_id_down = ImGui::DockBuilderSplitNode(
        dock_id_main, ImGuiDir_Down, 0.5f, nullptr, &dock_id_main);
    ImGuiID dock_id_right = ImGui::DockBuilderSplitNode(
        dock_id_main, ImGuiDir_Right, 0.5f, nullptr, &dock_id_main);
    ImGuiID dock_id_main_bottom = ImGui::DockBuilderSplitNode(
        dock_id_main, ImGuiDir_Down, 0.5f, nullptr, &dock_id_main);
  ...
  if (!simulation_results.timeline.empty()) {
    ...
    for (int i = 1; i <= simulation_results.timeline.size(); ++i) {
      if (i == simulation_results.timeline.size() ||
          simulation_results.timeline[i].pid != current_pid) {
...
void RenderUI() {
  ...
  ImGui::Text("%s", (add_process_status_msg.length() == 0)
                        ? ""
                        : add_process_status_msg.c_str());

  if (ImGui::Button("Add Process")) {
    int padd_status =
        CreateProcess(n_process_name, n_arrival_time, n_burst_time, n_priority);
    if (padd_status == PADD_EMPTY_NAME)
      add_process_status_msg = "Process name cannot be empty";
    else if (padd_status == PADD_EXISTS)
      add_process_status_msg =
          std::format("Process \"{}\" already exists", n_process_name);
    else if (padd_status == PADD_MAX_PROCESSES)
      add_process_status_msg = "Max number of processes reached (999)";
    else
      add_process_status_msg = std::format("Process \"{}\" (PID: {}) added",
                                           n_process_name, n_pid - 1);
  }
  ImGui::SameLine(0, -1);
  ...
  if (ImGui::Button("Add Random Process")) {
    ...
    if ((CreateProcess(n_process_name.c_str(), n_arrival_time, n_burst_time,
                       n_priority)) == PADD_MAX_PROCESSES)
      add_process_status_msg = "Max number of processes reached (999)";
    else
      add_process_status_msg =
          std::format("Random process (PID: {}) added", n_pid - 1);
  }
  ...
  if (ImGui::Button("Add TOO MANY Processes")) {
    ...
    for (int i = 0; i < 500; ++i) {
      ...
      int padd_status = CreateProcess(n_process_name.c_str(), n_arrival_time,
                                      n_burst_time, n_priority);
    }
    if (padd_status == PADD_MAX_PROCESSES)
      add_process_status_msg = "Max number of processes reached (999)";
    else
      add_process_status_msg =
          "A ridiculous amount of processes have been added";
  }
  ImGui::End();
  /*
   * END - Create process
  ...
  if (ImGui::Button("Run Simulation")) {
    ...
    if (simulation_status == SIM_ALG_NULLPTR)
      ...
      simulation_status_msg = std::format(
          "Successfully ran {}", algorithms[selected_algorithm_idx].ident);
    }
  };
  ...
  if (!timeline.empty()) {
    ...
    if (ImGui::BeginTable("Gantt Chart", 2, results_table_flags,
                          ImVec2(0.0f, 0.0f))) {
      ImGui::TableSetupColumn("Process", ImGuiTableColumnFlags_WidthFixed,
                              200.0f);
      ...
      ImGui::Text("Response time: %f, Turnaround time: %f, Waiting time: %f",
                  simulation_results.avg_response_time,
                  simulation_results.avg_turnaround_time,
                  simulation_results.avg_waiting_time);
      ImGui::Separator();
      for (Process &p : process_stats) {
        ImGui::Text("%s (PID: %d): Completed at %d, with response time of %d",
                    p.name.c_str(), p.pid, p.completion_time, p.response_time);
      }
    }
  }
  ImGui::End();
