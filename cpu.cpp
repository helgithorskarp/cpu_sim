...
result_simulation Cpu::run_scheduling_sim(Algorithm &scheduling_algorithm) {
  ...
  for (int i = 0; i < time_line_length; i++) {
    ...
    /// /// send running process, and unit time num to algorithm, and the
    /// procceses that arrived
    Process *res =
        scheduling_algorithm.step(i, running_process, arrived_procceses);
  ...
  result.avg_response_time =
      static_cast<double>(total_response_sum) / response_count;
  result.avg_turnaround_time =
      static_cast<double>(total_turnaround_sum) / total_completed;
  result.avg_waiting_time =
      static_cast<double>(total_waiting_sum) / total_completed;
