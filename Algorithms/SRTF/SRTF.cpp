...
Process *SRTF::step(int current_time, Process *running,
  ...
  /// if cpu is not empty, loop trough all the newly arrived process and see if
  /// any of them have smaller remaining time than the currently running
  /// proccess
  ...
  if (!is_cpu_empty) {
    ...
    for (auto &p : arrived) {
      if (p->remaining_time > 0 &&
          p->remaining_time < new_candidate->remaining_time) {
        new_candidate = p;
      }
    }
  } else if (is_cpu_empty &&
             !existing_procceses
                  .empty()) { /// If cpu is empty, Loop trough all procceses
                              /// and pick the one that has the lowest
                              /// remaining time left
    for (auto &p : existing_procceses) {
      ...
      if (p->remaining_time < new_candidate->remaining_time &&
          p->remaining_time > 0) {
        new_candidate = p;
      }
    }
  ...
  existing_procceses.erase(std::remove(existing_procceses.begin(),
                                       existing_procceses.end(), new_candidate),
